...
- **Storing passwords in plain text or in the database without hashing.** If AI creates a users table with a column called "password" that stores readable text, that's a critical security failure. Passwords must always be hashed — scrambled into an unreadable format that can't be reversed.
...
Ready? Take the Auth & Permissions Exam →
